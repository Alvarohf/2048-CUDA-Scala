...
void modoManual(int* p_tablero, int p_num_columnas, int p_num_filas, bool p_dificultad, int p_semillas_baja, int p_semillas_alta, int P_TILE_WIDTH);
void modoAutomatico(int* p_tablero, int p_num_columnas, int p_num_filas, bool p_dificultad, int p_semillas_baja, int p_semillas_alta, int P_TILE_WIDTH);
...
int obtenerCaracteristicasCUDA(int p_num_columnas, int p_num_filas);
...
__global__ void  moverAbajo(int* M_dev, int p_num_columnas, int p_num_filas, int* p_puntuacion_dev, int P_TILE_WIDTH) {
	...
	//Posicion del hilo del bloque en la matriz
	int posicion = fila + col * p_num_columnas;
	int aux = M_dev[posicion];
	//Nos aseguramos de que se bifurquen los hilos y solo trabajen sobre la matriz aquellos correspondientes
	if (fila < p_num_columnas && col < p_num_filas) {
		for (int i = 0; i < p_num_columnas; i++)
		{
			//El ultimo hilo es el que mas valores deja modificados
			if ((posicion % p_num_columnas) == i) {
				if (aux != 0) {
					M_dev[posicion] = aux;
					M_dev[posicion + 1] = aux;
					M_dev[posicion + p_num_columnas] = aux;
					M_dev[posicion + 1 + p_num_columnas] = aux;
				}
			}
		}
			...
			if ((posicion / p_num_columnas) == k) {
				...
				if (posicion >= p_num_columnas) {
					...
					if (M_dev[posicion - p_num_columnas] >= 0) {
						...
						if (M_dev[posicion] == M_dev[posicion - p_num_columnas])
						{
							suma = M_dev[posicion] * 2;
							//Si son iguales se suman los valores dando como resultado el doble
							M_dev[posicion] = M_dev[posicion] * 2;
...
* @param p_num_filas Numero de filas del tablero
*/
__global__ void  moverArriba(int* M_dev, int p_num_columnas, int p_num_filas, int* p_puntuacion_dev, int P_TILE_WIDTH) {
	//Obtendra la suma que se añadir a la puntuacion
	int suma = 0;
	// Calculamos la fila en la matriz correspondiente al hilo y su bloque
	int fila = blockIdx.y*P_TILE_WIDTH + threadIdx.y;
	...
	//Nos aseguramos de que se bifurquen los hilos y solo trabajen sobre la matriz aquellos correspondientes
	if (fila < p_num_columnas && col < p_num_filas) {
		//Primero quitamos los 0 de la matriz para ello cada hilo en cada bloque ira subiendo los valores
		//de la matriz de abajo si no son 0 esto se hara varias veces hasta asegurarse de que este completo
		for (int i = 0; i < p_num_filas; i++) {
			//En caso de que el valor del hilo es 0 se mueve
			if ((M_dev[posicion] == 0)) {
				//Hay que asegurarse de no pasarse de rango de la columna
				if ((posicion + (p_num_columnas)) < (p_num_columnas*p_num_filas)) {
					//Si es distinto de 0 el valor anterior se procede a bajar
					if ((M_dev[posicion + p_num_columnas] > 0)) {
		...
		for (int k = 0; k < p_num_filas; k++) {
			...
			if (((fila + col * p_num_columnas) / p_num_columnas) == k) {
...
__global__ void  moverIzquierda(int* M_dev, int p_num_columnas, int p_num_filas, int* p_puntuacion_dev, int P_TILE_WIDTH) {
	//Obtendra la suma que se añadir a la puntuacion
	int suma = 0;
	...
	if (fila < p_num_columnas && col < p_num_filas) {
		...
		for (int i = 0; i < p_num_columnas; i++) {
			...
			if ((M_dev[posicion] == 0)) {
				...
				if (!(M_dev[posicion + 1] <= 0) && ((posicion + 1) < p_num_columnas*((posicion / p_num_columnas) + 1))) {
					//Se reemplaza el valor de la casilla actual por el de despues
					M_dev[posicion] = M_dev[posicion + 1];
					//Se deja la casilla posterior a 0 para que pueda ser ocupada por otros valores
					M_dev[posicion + 1] = 0;
				}
			}
		...
		for (int k = 0; k < p_num_columnas; k++) {
			if ((posicion % p_num_columnas) == k) {
				...
				if (M_dev[posicion + 1] >= 0) {
					//Si los valores son iguales se suma
					if ((posicion + 1) < p_num_columnas*((posicion / p_num_columnas) + 1)) {


						if (M_dev[posicion] == M_dev[posicion + 1])
						{
							suma = M_dev[posicion] * 2;
...
__global__ void  moverDerecha(int* M_dev, int p_num_columnas, int p_num_filas, int* p_puntuacion_dev, int P_TILE_WIDTH) {
	//Obtendra la suma que se añadir a la puntuacion
	int suma = 0;
	//Movemos los valores distintos de 0 a la izquierda
	int fila = blockIdx.y*P_TILE_WIDTH + threadIdx.y;
	// Calculamos la columna en la matriz correspondiente al hilo y su bloque
	int col = blockIdx.x*P_TILE_WIDTH + threadIdx.x;
	//Posicion actual del hilo en la matriz
	int posicion = fila + col * p_num_columnas;
	...
	if (fila < p_num_columnas && col < p_num_filas) {
		for (int i = 0; i < p_num_columnas; i++) {
			...
			if ((M_dev[posicion] == 0)) {
				//Si es distinto de 0 el valor anterior se procede a bajar siempre y cuando sea de la misma fila
				if (posicion >= 1) {
					if (!(M_dev[posicion - 1] <= 0) && ((posicion - 1) >= p_num_columnas * ((posicion / p_num_columnas)))) {
						//Se reemplaza el valor de la casilla actual por el de despues
						M_dev[posicion] = M_dev[posicion - 1];
						//Se deja la casilla posterior a 0 para que pueda ser ocupada por otros valores
						M_dev[posicion - 1] = 0;
...
int main(int argc, char** argv)
{
	...
	if (argc > 3) {
		...
		TILE_WIDTH = obtenerCaracteristicasCUDA(num_columnas, num_filas);
		getch();
		//Modo automatico
		if (strcmp(argv[1], "-a") == 0) {
			modoAutomatico(tablero, num_columnas, num_filas, dificultad, SEMILLAS_ALTA, SEMILLAS_BAJA, TILE_WIDTH);
		}
		else if (strcmp(argv[1], "-m") == 0)
		{
			modoManual(tablero, num_columnas, num_filas, dificultad, SEMILLAS_ALTA, SEMILLAS_BAJA, TILE_WIDTH);
		}
		else {
	...
	tablero = inicializarMatriz(num_columnas, num_filas, dificultad, SEMILLAS_ALTA, SEMILLAS_BAJA);
	imprimirMatrizVector(tablero, num_columnas, num_filas);
	int puntuacion = 0;
	int* puntuacion_dev = 0;
	int* tablero_dev;
	//Declaramos la dimension de CUDA
	dim3 dimBlock(TILE_WIDTH, TILE_WIDTH);
	//Para determinar los bloques dividimos la matriz con nuestra tesela, redondeando para arriba
	//Se debe redondear para poder ejecutarla completa, aunque esto implica hilos que no hacen nada
	dim3 dimGrid((int)ceil(((float)num_columnas / TILE_WIDTH)), (int)ceil(((float)num_filas / TILE_WIDTH)));
	//Reservamos memoria global en la grafica
	int size = num_columnas * num_filas;
	cudaMalloc((void**)&tablero_dev, size * sizeof(int));
	cudaMalloc((void**)&puntuacion_dev, sizeof(int));

	cudaMemcpy(tablero_dev, tablero, size * sizeof(int), cudaMemcpyHostToDevice);
	moverArriba << <dimGrid, dimBlock >> > (tablero_dev, num_columnas, num_filas, puntuacion_dev, TILE_WIDTH);
	cudaMemcpy(tablero, tablero_dev, size * sizeof(int), cudaMemcpyDeviceToHost);
...
void modoAutomatico(int* p_tablero, int p_num_columnas, int p_num_filas, bool p_dificultad, int p_semillas_baja, int p_semillas_alta, int P_TILE_WIDTH)
{
	...
	int* tablero_dev;
	//Declaramos la dimension de bloque de hilos en CUDA
	dim3 dimBlock(P_TILE_WIDTH, P_TILE_WIDTH);
	//Para determinar los bloques dividimos la matriz con nuestra tesela, redondeando para arriba
	//Se debe redondear para poder ejecutarla completa, aunque esto implica hilos que no hacen nada
	dim3 dimGrid((int)ceil(((float)p_num_columnas / P_TILE_WIDTH)), (int)ceil(((float)p_num_filas / P_TILE_WIDTH)));
	//Reservamos memoria global en la grafica
	cudaMalloc((void**)&tablero_dev, size * sizeof(int));
	cudaMalloc((void**)&puntuacion_dev, sizeof(int));
	do {
		...
		case 0: printf("\n Direccion: Arriba\n");
			...
			moverArriba << <dimGrid, dimBlock >> > (tablero_dev, p_num_columnas, p_num_filas, puntuacion_dev, P_TILE_WIDTH);
		...
		case 1: printf("\n Direccion: Izquierda\n");
			cudaMemcpy(tablero_dev, p_tablero, size * sizeof(int), cudaMemcpyHostToDevice);
			moverIzquierda << <dimGrid, dimBlock >> > (tablero_dev, p_num_columnas, p_num_filas, puntuacion_dev, P_TILE_WIDTH);
			cudaMemcpy(p_tablero, tablero_dev, size * sizeof(int), cudaMemcpyDeviceToHost);
			//Obtenemos la puntuacion despues del movimiento
			cudaMemcpy(&puntuacion, puntuacion_dev, sizeof(int), cudaMemcpyDeviceToHost);
			//Despues de realizar el movimiento se rellena con valores
		...
		case 2: printf("\n Direccion: Abajo\n");
			cudaMemcpy(tablero_dev, p_tablero, size * sizeof(int), cudaMemcpyHostToDevice);
			moverAbajo << <dimGrid, dimBlock >> > (tablero_dev, p_num_columnas, p_num_filas, puntuacion_dev, P_TILE_WIDTH);
			cudaMemcpy(p_tablero, tablero_dev, size * sizeof(int), cudaMemcpyDeviceToHost);
			//Obtenemos la puntuacion despues del movimiento
			cudaMemcpy(&puntuacion, puntuacion_dev, sizeof(int), cudaMemcpyDeviceToHost);
			//Despues de realizar el movimiento se rellena con valores
		...
		case 3: printf("\n Direccion: Derecha\n");
			cudaMemcpy(tablero_dev, p_tablero, size * sizeof(int), cudaMemcpyHostToDevice);
			moverDerecha << <dimGrid, dimBlock >> > (tablero_dev, p_num_columnas, p_num_filas, puntuacion_dev, P_TILE_WIDTH);
			cudaMemcpy(p_tablero, tablero_dev, size * sizeof(int), cudaMemcpyDeviceToHost);
			//Obtenemos la puntuacion despues del movimiento
			cudaMemcpy(&puntuacion, puntuacion_dev, sizeof(int), cudaMemcpyDeviceToHost);
			//Despues de realizar el movimiento se rellena con valores
...
int comprobarCasillasVacias(int * p_tablero, int p_num_columnas, int p_num_filas) {
	...
	for (int i = 0; i < p_num_filas; i++)
	{
		for (int j = 0; j < p_num_columnas; j++)
		{
			...
			else if (p_tablero[i*p_num_columnas + j] == 16384) {
...
/**
* Obtiene las caracteristicas de la grafica e indica si puede correr la matriz y devuelve la tesela adecuada
* @param p_num_columnas numero de columnas del tablero
* @param p_num_filas numero de filas del tablero
* @return tesela que se usara en los movimientos
*/
int obtenerCaracteristicasCUDA(int p_num_columnas, int p_num_filas) {
	cudaDeviceProp prop;
	...
	if (prop.maxThreadsPerMultiProcessor < p_num_columnas*p_num_filas) {
	...
	if (prop.totalGlobalMem < (p_num_columnas*p_num_filas * sizeof(int) + sizeof(int))) {
		printf("Error no hay suficiente memoria global para ejecutar la matriz");
		exit(-1);
	}
	//Comparacion en porcentajes
	printf("Porcentaje de aprovechamiento de hilos por SM: %.2f %% \n", ((double)(p_num_columnas*p_num_filas) / prop.maxThreadsPerMultiProcessor) * 100);
	//Debe de entrar en 1 bloque de la grafica y ser multiplo de 32 para aprovechar los WARP sin dejarlos vacios
	//Normalmente los mejores resultados los da con teselas de 16, que es la mas versatil
	//Si no tendremos 32 que puede ser empleada en cualquier grafica de capability mayor a 1
	if (prop.major == 1) {
		tile_width = 16;
	}
	return tile_width;
